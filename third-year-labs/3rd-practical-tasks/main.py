...
def create_student_profile():
    ...
    while True:
        try:
            input_list_of_desired_grades = list(
                map(
                    float,
                    input(
                        "Enter the list of desired grades (comma-separated): "
                    ).split(',')))
            if len(input_list_of_desired_grades) != len(
                    input_list_of_subjects):
                raise ValueError(
...
def save_student_profile(student_profile):
    ...
    file_path = f"{student_profile.name}.{file_format}"
